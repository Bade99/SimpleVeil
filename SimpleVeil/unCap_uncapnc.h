...
//TODO(fran): parent will send msg on close,min,max,restore,etc for the child to do special handling, if return value is 0 then we handle it
...
//Starts at idx 0 which would normally correspond to the close button, idx 1 maximize, and idx 2 minimize
RECT UNCAPNC_calc_btn_rc(unCapNcProcState* state, int idx) {
	Assert(idx >= 0);
	RECT r; GetClientRect(state->wnd, &r);
	RECT rc{ r.right - (idx+1) * state->caption_btn.cx, r.top, r.right - idx * state->caption_btn.cx, r.top + state->caption_btn.cy };
	return rc;
}
...
LRESULT CALLBACK UncapNcProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_CREATE:
	{
		...
		if (state->settings->can_minimize) {
		...
		if (state->settings->can_maximize) {
			RECT btn_max_rc = UNCAPNC_calc_btn_rc(state, 1);
		...
		if (state->settings->can_close) {
			RECT btn_close_rc = UNCAPNC_calc_btn_rc(state,0);
	...
	case WM_SIZE:
	{
		...
		RECT btn_min_rc = UNCAPNC_calc_btn_rc(state,1); MoveWindow(state->btn_min, btn_min_rc.left, btn_min_rc.top, RECTWIDTH(btn_min_rc), RECTHEIGHT(btn_min_rc), TRUE);//TODO(fran): I dont really need to ask for repaint do I?
#if 0 //TODO(fran): parametric
...
#endif
		RECT btn_close_rc = UNCAPNC_calc_btn_rc(state,0); MoveWindow(state->btn_close, btn_close_rc.left, btn_close_rc.top, RECTWIDTH(btn_close_rc), RECTHEIGHT(btn_close_rc), TRUE);
	...
	case WM_COMMAND:
	{
		...
		{
			if (!SendMessage(state->client, WM_COMMAND, MAKELONG(UNCAPNC_MINIMIZE, 0), 0)) //We ask the client in case it wants to do something different, we expect !=0 in case they do the handling
		...
		case UNCAPNC_CLOSE:
		{
			if (!SendMessage(state->client, WM_COMMAND, MAKELONG(UNCAPNC_CLOSE, 0), 0)) //We ask the client in case it wants to do something different, we expect !=0 in case they do the handling
...
ATOM init_wndclass_unCap_uncapnc(HINSTANCE inst) {
	...
	wcex.hInstance = inst;
	wcex.hIcon = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(UNCAP_ICO_LOGO)); //TODO(fran): LoadImage to choose the best size
